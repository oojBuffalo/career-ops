...
if (/Language Rule/i.test(batchPrompt) && /write all human-facing output in English/i.test(batchPrompt)) {
  pass('batch prompt pins worker prose to English');
} else {
  fail('batch prompt does not pin worker prose to English');
}
...
if (
  offerMode.includes('### Work-authorization check') &&
  offerMode.includes('⛔ **No sponsorship:** JD states "{verbatim JD line}" and role is outside your authorized_in') &&
...
// --- Block G agency licensing check (#2037) ---
{
  ...
  // 2. offer.md carries the agency-licensing section with the agency-mediated
  //    trigger, registry pointer, tracker-note suggestion, and jurisdiction derivation
  const alStart = offerMode.indexOf('Agency Licensing Check');
  const alEnd = offerMode.indexOf('### Output format:', Math.max(alStart, 0));
  const alSection = alStart >= 0 && alEnd > alStart ? offerMode.slice(alStart, alEnd) : '';
  if (
...
if (
  offerMode.includes('company-history.mjs') &&
...
// Note: Block G's reposting signal in _shared.md/offer.md is intentionally
...
{
  const jdPathModes = ['modes/offer.md', 'modes/auto-pipeline.md', 'modes/pipeline.md', 'modes/scan.md'];
...
try {
  ...
  if (
    ...
    prepMode.includes('`outreach`')
  ) {
    pass('interview-prep URL entry scope guard: no tracker writes, CV generation stays in pdf, contact automation stays in outreach');
  } else {
    fail('interview-prep URL entry missing the out-of-scope guard (tracker read-only / pdf / outreach)');
  }
} catch (e) {
...
// --- immigration-status requirement overreach (#2033): table + offer Block G + apply Step 5d ---
{
  ...
  if (sigSection && stepSection && facingLines.length >= 2 && assertive.length === 0) {
    ...
    fail(`immigration-status phrasing discipline broken: ${assertive.length ? `employer-directed assertion in rendered template: ${assertive[0].trim().slice(0, 80)}` : 'expected blockquote templates in both the offer signal and apply Step 5d'} (#2033)`);
  }
...
// --- jurisdiction-prohibited content signal (#2018): table + offer Block G + apply Step 5c ---
{
  ...
  if (
    offerMode.includes('**12. Jurisdiction-Prohibited Content**') &&
    offerMode.includes('templates/jurisdiction-prohibited-content.yml') &&
    offerMode.includes('⚠️ **Jurisdiction-prohibited content signal:**') &&
  ...
  const signal9 = offerMode.slice(
    offerMode.indexOf('**12. Jurisdiction-Prohibited Content**'),
    offerMode.indexOf('### Output format:')
  );
